...
st.title('Custom Machine Learning Tools')
st.success(
    "This module of **AIDrugApp v1.2.4** includes four components: 'Mol Identifier', 'DesCal', 'AutoDL', and 'Auto-Multi-ML' which provide users"
    "with free access to molecular identification using SMILES and compound names, similarity search, descriptor calculation, building of ML/DL QSAR models and their usage in predicting new data.")

st.sidebar.header("Menu")

add_selectbox = st.sidebar.radio("Select ML tool", ("Auto-Multi-ML", "Auto-DL", "DesCal", "Mol_Identifier"))

st.sidebar.header('⚙️USER INPUT PANEL')
...
if add_selectbox == 'Mol_Identifier':
    ...
    if add_selectbox1 == 'Multiple molecules (Batch)':
        if add_selectbox2 == "Name to SMILE":
            # Sidebar
            with st.sidebar.subheader('2. Upload your CSV data'):
